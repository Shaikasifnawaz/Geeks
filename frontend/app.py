...
def render_dashboard_template(
    ...
    global_search = st.text_input("Global search input", placeholder="Search teams, players, venues, rankings...", key="global_search", label_visibility="collapsed")
    ...
    with col1:
        st.markdown(
            ...
            <div class='dashboard-card'>
                ...
                <p style='margin-top: 1rem;'><strong>Frontend:</strong></p>
...
def render_ai_chat_panel():
    """Render the AI chat panel with a fixed input and send icon."""
    st.markdown("<div class='section-spacing'>", unsafe_allow_html=True)
    st.markdown("### 🎓 NCAAFB Database Professor")
    st.markdown("<p style='color: #7a83a7; margin-bottom: 1.5rem;'>Your expert guide to college football data analytics</p>", unsafe_allow_html=True)
    
    # Initialize chat history in session state if not present
    if "ai_chat_history" not in st.session_state:
        st.session_state.ai_chat_history = []
    
    # Initialize flags to track if we need to process a new message and show loading
    if "process_next_message" not in st.session_state:
        st.session_state.process_next_message = False
    ...
    for message in st.session_state.ai_chat_history:
        if message["role"] == "user":
            ...
            # Only display assistant messages that have content
            if message["content"]:
                st.markdown(
                    f"""
                    <div style="display: flex; justify-content: flex-start; margin-bottom: 15px;">
                        ...
                        <div style="background: #f0f2f6; color: #333; border-radius: 18px 18px 18px 4px; padding: 12px 16px; max-width: 80%;">
                            <div style="font-weight: 500; font-size: 0.85rem; margin-bottom: 4px; color: #F1A208;">Database Professor</div>
                            <div>{message["content"]}</div>
                        </div>
                    </div>
                    """,
    ...
    # Show loading indicator in chat area if we're processing a message
    ...
    with st.form(key="ai_chat_form", clear_on_submit=True):
        # Use columns to create a fixed input with send button
        col1, col2 = st.columns([6, 1])
        
        with col1:
            user_input = st.text_input(
                "AI chat input",
                placeholder="Ask me anything about NCAAFB teams, players, stats, rankings, etc...",
                key="ai_chat_input",
        ...
        if send_button and user_input:
            ...
            # Set flags to process the next message and show loading
            st.session_state.process_next_message = True
            st.session_state.show_loading = True
            
            # Rerun to update the UI and show user message and loading immediately
            st.rerun()
    
    # Handle API call after user message is displayed
    if st.session_state.process_next_message:
        # Reset the flags
        st.session_state.process_next_message = False
        st.session_state.show_loading = False
        
        # Get the last user message
        user_message = st.session_state.ai_chat_history[-1]["content"]
        
        # Query LLM API
        response = query_llm_api(user_message)
        ...
        # Rerun to update the UI with bot response
        st.rerun()
